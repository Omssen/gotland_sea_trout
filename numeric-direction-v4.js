...
(function(){
  ...
  numericSmoothCurrentLayer=function(samples){
    ...
    return new G({pane:'seaDataPane',tileSize:256,opacity:1,updateWhenIdle:false,updateWhenZooming:true,keepBuffer:2,noWrap:true,bounds:[[GOTLAND_RENDER_DOMAIN.s,GOTLAND_RENDER_DOMAIN.w],[GOTLAND_RENDER_DOMAIN.n,GOTLAND_RENDER_DOMAIN.e]]})
  };
  ...
  function vectors(samples){
    ...
    return new V({pane:'currentVectorPane',tileSize:256,opacity:1,updateWhenIdle:false,updateWhenZooming:true,keepBuffer:2,noWrap:true,bounds:[[GOTLAND_RENDER_DOMAIN.s,GOTLAND_RENDER_DOMAIN.w],[GOTLAND_RENDER_DOMAIN.n,GOTLAND_RENDER_DOMAIN.e]]})
  }
  drawNumericCurrent=function(samples,layer){
    liveSamples=samples;
    liveField=numericSmoothCurrentLayer(samples);
    liveVectors=vectors(samples);
    liveField.addTo(layer);liveVectors.addTo(layer);
  };
  function forceCurrentRedraw(){
    if(!liveSamples||!active?.current||!map.hasLayer(currentLayer))return;
    clearTimeout(redrawTimer);
    redrawTimer=setTimeout(()=>{
      if(!active?.current||!map.hasLayer(currentLayer))return;
      if(liveField&&currentLayer.hasLayer(liveField))liveField.redraw();
      if(liveVectors&&currentLayer.hasLayer(liveVectors))liveVectors.redraw();
    },60);
  }
