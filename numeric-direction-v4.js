/* v6.2.4k SMOOTH FIELD + explicit compass arrows
   Stable low-zoom renderer: tile creation is clipped in createTile(), not by
   Leaflet GridLayer bounds. Performance: keep current tiles static during
   pinch/zoom and redraw only after interaction has settled. */
(function(){
  let liveField=null,liveVectors=null,liveSamples=null,redrawTimer=null;
  ...
  function fieldFactor(z){return z>=14?7:z>=12?9:z>=10?12:16}
  numericSmoothCurrentLayer=function(samples){
    const G=L.GridLayer.extend({createTile:function(coords){
      ...
      const ctx=tile.getContext('2d',{alpha:true}),factor=fieldFactor(coords.z),margin=3,low=document.createElement('canvas'),lw=Math.ceil(size.x/factor)+margin*2,lh=Math.ceil(size.y/factor)+margin*2;low.width=lw;low.height=lh;
      ...
      lc.putImageData(img,0,0);ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='low';ctx.filter='blur(2px)';ctx.drawImage(low,margin,margin,lw-margin*2,lh-margin*2,0,0,size.x,size.y);ctx.filter='none';eraseLandFromTile(ctx,coords,size);return tile
    }});
    return new G({pane:'seaDataPane',tileSize:256,opacity:1,updateWhenIdle:true,updateWhenZooming:false,keepBuffer:1,noWrap:true})
  };
  ...
  function vectorGap(z){return z>=14?58:z>=12?66:z>=10?76:90}
  function vectors(samples){
    const V=L.GridLayer.extend({createTile:function(coords){
      ...
      eraseLandFromTile(ctx,coords,size);return tile
    }});
    return new V({pane:'currentVectorPane',tileSize:256,opacity:1,updateWhenIdle:true,updateWhenZooming:false,keepBuffer:1,noWrap:true})
  }
  drawNumericCurrent=function(samples,layer){liveSamples=samples;liveField=numericSmoothCurrentLayer(samples);liveVectors=vectors(samples);liveField.addTo(layer);liveVectors.addTo(layer)};
  function forceCurrentRedraw(){if(!liveSamples||!active?.current||!map.hasLayer(currentLayer))return;clearTimeout(redrawTimer);redrawTimer=setTimeout(()=>{if(!active?.current||!map.hasLayer(currentLayer))return;if(liveField&&currentLayer.hasLayer(liveField))liveField.redraw();if(liveVectors&&currentLayer.hasLayer(liveVectors))liveVectors.redraw()},180)}
  map.on('zoomend moveend resize',forceCurrentRedraw);
