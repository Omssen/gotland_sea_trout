/* v6.2.4k SMOOTH FIELD + explicit compass arrows
   ...
   Leaflet GridLayer bounds, so current tiles are still created after zooming out. */
(function(){
  ...
  numericSmoothCurrentLayer=function(samples){
    const G=L.GridLayer.extend({createTile:function(coords){
      ...
      lc.putImageData(img,0,0);ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='medium';ctx.filter='blur(2.5px)';ctx.drawImage(low,margin,margin,lw-margin*2,lh-margin*2,0,0,size.x,size.y);ctx.filter='none';eraseLandFromTile(ctx,coords,size);return tile
    }});
    return new G({pane:'seaDataPane',tileSize:256,opacity:1,updateWhenIdle:false,updateWhenZooming:true,keepBuffer:2,noWrap:true})
  };
  ...
  function vectors(samples){
    ...
    return new V({pane:'currentVectorPane',tileSize:256,opacity:1,updateWhenIdle:false,updateWhenZooming:true,keepBuffer:2,noWrap:true})
  }
