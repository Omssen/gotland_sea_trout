...
test('ranking coverage: eight successes and total failure use uniform fallback without caching',async()=>{for(const limit of [8,0,124]){const h=rankingSpatialSetup(),fetch=h.c.fetchCurrentPoint;let calls=0,contexts=[];h.c.fetchCurrentPoint=(p,t)=>calls++<limit?fetch(p,t):Promise.reject(Error('missing'));const score=h.c.scoreDetailed;h.c.scoreDetailed=(w,m,s,ctx)=>{contexts.push(ctx);return score(w,m,s,ctx)};await h.run();assert.equal(calls,125);assert.equal(contexts.length,25);assert.ok(contexts.every(x=>x===null));assert.equal(h.c.currentNumericCache.size,0)}});
